...
<%@ page import="com.nimbusds.tenants.manager.*" %>
<%@ page import="com.nimbusds.oauth2.sdk.id.*" %>
<%@ page import="com.nimbusds.openid.connect.provider.*" %>
<%@ page import="com.nimbusds.openid.connect.provider.checksession.*" %>
...
try {
    ...
    issuer = tenantCtx.getIssuer();
    cookieName = CheckSessionConfiguration.from(tenantCtx).cookieName;
...
/* Checks the session ID with the Connect2id server backend */
function checkSessionWithBackend(sid, targetWindow, eventOrigin) {

    var oReq = new XMLHttpRequest();
    oReq.addEventListener("load", function() {
        targetWindow.postMessage(oReq.responseText, eventOrigin);
    });
    oReq.open("POST", "<%= issuer %>/check-session-backend/rest/v1");
...
function receiveMessage(event) {
    ...
    <%
        if (cookieName == null) {
            // Check session iframe is disabled, return error to client
            %>
            console.log("Check session endpoint disabled");
    ...
    try {
        ...
        if (expectedHash === receivedHash) {
            // Check session ID with the Connect2id server backend to prevent
            // false "unchanged" on expired session ID when cookie is still
            // present
            checkSessionWithBackend(sid, event.source, event.origin);
        } else {
            event.source.postMessage("changed", event.origin);
        }
